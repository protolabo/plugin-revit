    ...
    internal class ExportInfo
    {
        public static Result ProcessElements(ExternalCommandData commandData)
        {
            ...
            foreach (var viewId in window.SelectedViewIds)
            {
                ...
                foreach (var wall in walls)
                {
                    ...
                    var wallObj = new
                    {
                        ...
                        openings = doorsAndWindows
                            ...
                            .Select(inst => SubFunctions.GetOpenInfo.ExtractOpeningInfo(
                                inst,
                ...
                SubFunctions.WallOpen.ProcessWallOpen($"elements_{viewName}", $"muros_divididos_{viewName}");

                SubFunctions.ImageCreator.PrepareImageAndFiles(commandData, outputDir, window.SelectedViewIds);
            }
